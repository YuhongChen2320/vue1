<template>
	<div class="hello">
		...
		<el-table :data="tables" :stripe="true" :border="true" width="100%">
			...
			<el-table-column label="寄件人姓名" prop="recipientName"></el-table-column>
			<el-table-column label="收件人姓名" prop="senderName"></el-table-column>
			...
			<el-table-column label="网点" fixed="right" width="180">
				<template slot-scope="scope">
					<el-button type="primary" @click="showStationInfo(scope.$index, scope.row)">站点信息</el-button>
				</template>
			</el-table-column>
			<el-table-column label="操作" fixed="right" width="180">
				<template slot-scope="scope">
					<el-button type="primary" @click="modifyData(scope.$index, scope.row)">修改</el-button>
					<el-button type="danger" @click="deleteData(scope.$index, tableData)">取消</el-button>
				</template>
			</el-table-column>
		</el-table>

		<el-dialog :visible.sync="StationInfoVisible">
			<el-form :model="stationForm" disabled="isDisabled">
	...
	methods: {
		...
		async deleteData(index, row) {
			let res = await axios.post(
				"http://localhost:8866/client/deleteExpressByExpressNo",
		...
		async sumbitEditRow() {
			...
			try {
				let res = await axios.post(
					"http://localhost:8866/client/updateExpress", qs.stringify(this.editForm),
				).then(res => {
					this.init();
					this.centerDialogVisible = false;
				});
				let res1 = await axios.post(
					"http://localhost:8866/client/updateStationExpress", qs.stringify
					({
		...
		async sumbitAddRow() {
			...
			try {
				let res = await axios.post(
					"http://localhost:8866/client/updateExpress", qs.stringify(this.addForm),
				).then(res => {
					this.isAddMembers = false;
					this.$router.go(0)
				});
		...
		async init() {
			...
			try {
				let res = await axios.post(
					"http://localhost:8866/client/getExpressBySender"
				).then(res => {
		...
		async initStation() {
			try {
				let res = await axios.post(
					"http://localhost:8866/client/getStations"
				).then(res => {
			...
			axios({
				method: 'post',
				url: 'http://localhost:8866/client/getStationByExpressList',
				headers: {
