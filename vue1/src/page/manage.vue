<template>
	<div class="hello">
		...
		<el-table :data="tables" :stripe="true" :border="true" width="100%">
			...
			<el-table-column label="快递员编号" prop="courierNo" ></el-table-column>
		...
		<el-dialog :visible.sync="StationInfoVisible">
			<el-form :label-position='right'  :model="stationForm">
		...
		<el-dialog :visible.sync="centerDialogVisible">
			<el-form :model="editForm">
				...
				<el-form-item label="快递员编号">
					<el-input v-model="editForm.courierNo"></el-input>
				</el-form-item>
	...
	methods: {
		...
		async deleteData(index, row) {
			let res = await axios.post(
				this.url+"/manager/deleteCourier",
				qs.stringify({ courierNo: this.tableData.at(index).courierNo })
			)
				.then(res => {
		...
		async sumbitAddRow() {
			...
			try {
				let res = await axios.post(
					this.url+"/manager/addCourier", qs.stringify(this.addForm),
				).then(res => {
		...
		async initStation() {
			...
			axios({
				method: 'post',
				url: this.url+"manager/getStationListByCourier",
				headers: {
